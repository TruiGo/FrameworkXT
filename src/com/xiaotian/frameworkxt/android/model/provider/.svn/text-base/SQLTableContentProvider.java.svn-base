...
public abstract class SQLTableContentProvider<EntityClass> extends ContentProvider {
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		uri = values.containsKey("_id") ? Uri.withAppendedPath(uri, String.valueOf(values.get("_id"))) : Uri.withAppendedPath(uri,
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		case URIMATCHED_CODE_TABLE:
			...
			return cursor;
		case URIMATCHED_CODE_ID:
			cursor = queryBuilder.query(db, projection == null ? getEntityProjects() : projection, "_id=?",
					new String[] { uri.getPathSegments().get(2) }, null, null, "_id ASC");
			cursor.setNotificationUri(getContext().getContentResolver(), uri);
			return cursor;
		default:
			return null;
	...
	/**
	 * @param URI
	 *            If database is set for dynamic the database name is from uri,other ignore.
