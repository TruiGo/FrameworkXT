...
public class JSONSerializer {
	...
	public <T extends Object> T deSerialize(String value, Class<?> baseClass) throws JSONSerializingException {
		...
		if (!(annotation instanceof JSONEntity)) throw new JSONSerializingException(JSONSerializingException.UNSUPEREXCEPTION);
		...
		try {
			for (Field field : fields) {
				...
				if (annotation instanceof JSONField) {
					...
					if (fieldAnnotation.value() != JSONField.DEFAULTVALUE) {
		...
		try {
			for (Field field : fields) {
				...
				if (fieldAnnotation instanceof JSONField) {
					...
					if (fieldAnnotation.name().equals(JSONField.DEFAULTNAME)) {
